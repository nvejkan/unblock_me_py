...
board_list = []
def go_deep(b):
    this_stage_board = b.board.copy()
    print('this_stage_board:')
    print(this_stage_board)
    for i in b.blocks.keys():
        block = b.blocks[i][0]
        start = b.blocks[i][1]
        
        for j in [b.move_up,b.move_down,b.move_left,b.move_right]:
            if j(block,start):
                found_flag = False
                for x in board_list:
                    if (x.board == b.board).all().all():
                        found_flag = True
                        break
                if not found_flag:
                    new_board = Board() #create new board obj
                    new_board.board = b.board.copy() #set the board
                    b.board = this_stage_board.copy() #reset the board
