...
class Board:
    ...
    def place_block(self,block,start,just_peek = False):
        ...
        if  self.chk_start_end_pos(start,end) \
            ...
            #print(start,end)
                ...
                peek_board = self.init_board()
                for i in peek_blocks.keys():
                    start = peek_blocks[i][1]
                    end = peek_blocks[i][2]
                    peek_board.loc[ self.rt.format(start[0]) : self.rt.format(end[0]) \
                                ,self.ct.format(start[1]) : self.ct.format(end[1]) ] = i
                    
                return peek_blocks,peek_board
    ...
    def move(self,block,start,move_direction,allow_direction,just_peek = False):
    ...
    def move_down(self,block,start,just_peek = False):
        '''
        old_start = start
        ...
        if block.direction == 'down' and self.chk_start_end_pos(start,end) \
            ...
            return False
        '''
        return self.move(block,start,'down','down',just_peek)
            
    def move_up(self,block,start,just_peek = False):
        return self.move(block,start,'up','down',just_peek)
            
    def move_right(self,block,start,just_peek = False):
        return self.move(block,start,'right','right',just_peek)
        
    def move_left(self,block,start,just_peek = False):
        return self.move(block,start,'left','right',just_peek)
